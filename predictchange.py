...
price = pd.read_csv('stock_data.csv')[['vol','amount','change']]
...
for t in range(num_epochs):
    ...
    if (t+1)%100==0:
        ...
        y_test_pred = scaler['change'].inverse_transform(y_test_pred.cpu().detach().numpy().reshape(-1, 1))
        y_test = scaler['change'].inverse_transform(y_test_lstm.cpu().detach().numpy().reshape(-1, 1))
...
# invert predictions
y_train_pred = scaler['change'].inverse_transform(y_train_pred.cpu().detach().numpy().reshape(-1, 1))
y_train = scaler['change'].inverse_transform(y_train_lstm.cpu().detach().numpy().reshape(-1, 1))
y_test_pred = scaler['change'].inverse_transform(y_test_pred.cpu().detach().numpy().reshape(-1, 1))
y_test = scaler['change'].inverse_transform(y_test_lstm.cpu().detach().numpy().reshape(-1, 1))
...
trainPredictPlot = np.empty_like(price['change'])[:,np.newaxis]
trainPredictPlot[:, :] = np.nan
trainPredictPlot[lookback-1:len(y_train_pred)+lookback-1, :] = y_train_pred

# shift test predictions for plotting
testPredictPlot = np.empty_like(price['change'])[:,np.newaxis]
testPredictPlot[:, :] = np.nan
testPredictPlot[len(y_train_pred)+lookback-1:len(price), :] = y_test_pred

original = scaler['change'].inverse_transform(price['change'].values.reshape(-1,1))
